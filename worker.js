...
async function setup(db){
  await db.batch([
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS platform_balances(currency TEXT PRIMARY KEY,balance REAL NOT NULL DEFAULT 0)`),
    db.prepare(`CREATE TABLE IF NOT EXISTS reserve_balances(currency TEXT PRIMARY KEY,balance REAL NOT NULL DEFAULT 0)`)
  ]);
  for(const d of [
  ...
  for(const c of CURRENCIES){await db.prepare(`INSERT OR IGNORE INTO platform_balances(currency,balance) VALUES(?,0)`).bind(c).run();await db.prepare(`INSERT OR IGNORE INTO reserve_balances(currency,balance) VALUES(?,0)`).bind(c).run()}
}
...
async function settle(req,env){let b=await req.json(),id=String(b.eventId||""),w=String(b.winner||"");if(!["home","draw","away"].includes(w))throw err("Rezilta pa valab");let r=await env.DB.prepare(`SELECT * FROM bets WHERE event_id=? AND status='open'`).bind(id).all(),groups={};for(const x of r.results)(groups[x.currency]??=[]).push(x);for(const[c,bs]of Object.entries(groups)){let total=bs.reduce((s,x)=>s+Number(x.stake),0),wins=bs.filter(x=>x.market===w),ws=wins.reduce((s,x)=>s+Number(x.stake),0),pool=round(total*.6),platform=0,agents={};for(const x of bs){if(x.agent){platform+=Number(x.stake)*.3;agents[x.agent]=(agents[x.agent]||0)+Number(x.stake)*.1}else platform+=Number(x.stake)*.4}for(const x of bs){let pay=x.market===w&&ws>0?round(pool*Number(x.stake)/ws):0,status=x.market===w&&ws>0?"won":"lost";if(pay)await env.DB.prepare(`UPDATE wallets SET balance=balance+? WHERE uid=? AND currency=?`).bind(pay,x.uid,c).run();await env.DB.prepare(`UPDATE bets SET status=?,payout=? WHERE id=?`).bind(status,pay,x.id).run()}await env.DB.prepare(`UPDATE platform_balances SET balance=balance+? WHERE currency=?`).bind(round(platform),c).run();if(ws===0)await env.DB.prepare(`UPDATE reserve_balances SET balance=balance+? WHERE currency=?`).bind(pool,c).run();for(const[k,v]of Object.entries(agents))await env.DB.prepare(`UPDATE agents SET commission_balance=commission_balance+? WHERE code=?`).bind(round(v),k).run()}return out({ok:true})}
